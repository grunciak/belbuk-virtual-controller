...
async function startServer() {
  ...
  const server = new ApolloServer({
    ...
    plugins: [
      ...
      // Plugin do logowania zapytań
      {
        async requestDidStart(requestContext) {
          const query = requestContext.request.query || '';
          // Loguj tylko Configuration query (od Belbuka)
          if (query.includes('configuration') || query.includes('Configuration')) {
            console.log('\n[BELBUK REQUEST]');
          ...
          return {
            async willSendResponse(requestContext) {
              ...
              if (query.includes('configuration') || query.includes('Configuration')) {
                ...
                // Pokaż pierwsze 1000 znaków odpowiedzi
                console.log('Response preview:', response.substring(0, 1000));
                if (requestContext.response.body?.singleResult?.errors) {
                  console.log('ERRORS:', JSON.stringify(requestContext.response.body.singleResult.errors));
                }
              }
            },
          };
